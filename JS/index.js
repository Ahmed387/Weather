...
if (navigator.geolocation) {
  navigator.geolocation.getCurrentPosition(function (pos) {
    xxx = pos.coords.latitude;
    yyy = pos.coords.longitude;
  });
}
...
async function GetData(V) {
  try {
    const response = await fetch(
      `https://api.weatherapi.com/v1/forecast.json?q=${V}&days=4&key=92d4820346b64139939105653240806`
    );
  ...
  <img class="" src="./cant-see-shit.gif" alt="errorgif">
  </div>`;
  }
}
...
<div class="one-div p-4">
<h2 class="text-center mb-4 text-nowrap">${All.location.name}</h2>
<p class="h5 fw-bold">High temp : ${
      All.forecast.forecastday[i].day.maxtemp_c
    }</span></h4>
<p class="h5 fw-bold">Low temp :  ${
      All.forecast.forecastday[i].day.mintemp_c
    }</p>
<div class="icon d-flex justify-content-center align-items-center flex-column">
<h5 class="text-center pt-3"> ${
      All.forecast.forecastday[i].day.condition.text
    }</h5>
<img src="${All.forecast.forecastday[i].day.condition.icon}" alt="">
...
<div class="text-center"> latitude : ${xxx || "no location"}</div>
<div class="text-center">longitude : ${yyy || "no location"}</div>
